        ...
        with st.spinner("Scanning..."):
            try:
                ...
                if scan_type == "Single IP":
                    ...
                    if result["ports"]:
                        ports_df = pd.DataFrame(result["ports"])
                ...
                try:
                    ...
                    with dettab1:
                        if result["ports"]:
                            ...
                            # Port pie chart
                            port_services = ports_df.groupby("service").size().reset_index(name="count")
                            fig = px.pie(port_services, values="count", names="service",
...
with tab4:
    ...
    if zombies:
        ...
        st.dataframe(zombies_df, use_container_width=True)
